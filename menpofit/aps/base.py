...
class GenerativeAPS(object):
    ...
    def __init__(
        ...
        n_appearance_components=None,
        can_be_incremented=False,
        verbose=False,
        batch_size=None,
    ):
        # Check parameters
        ...
        patch_normalisation = checks.check_callable(patch_normalisation, n_scales)
        ...
        self.shape_graph = checks.check_graph(
            shape_graph, UndirectedGraph, "shape_graph", n_scales
        )
        self.deformation_graph = checks.check_graph(
            deformation_graph, [DirectedGraph, Tree], "deformation_graph", n_scales
        )
        ...
        # Train APS
        self._train(
            images, increment=False, group=group, batch_size=batch_size, verbose=verbose
        )

    def _train(
        self, images, increment=False, group=None, batch_size=None, verbose=False
    ):
    ...
    def _train_batch(self, image_batch, increment=False, group=None, verbose=False):
        ...
        if None in self.deformation_graph:
            ...
            self.deformation_graph = [
                deformation_mst if g is None else g for g in self.deformation_graph
            ]
        ...
        for j in range(self.n_scales):
            ...
            if verbose:
                print_dynamic("{}Building deformation model".format(scale_prefix))
            ...
            if not increment:
                self.deformation_models.append(
                    self._build_deformation_model(
                        deformation_shapes, self.deformation_graph[j], verbose=verbose
                    )
                )
            else:
                self.deformation_models[j].increment(
                    deformation_shapes, verbose=verbose
                )

            # Obtain warped images
            warped_images = self._warp_images(
                scaled_images, scale_shapes, j, scale_prefix, verbose
            )
    ...
    def increment(self, images, group=None, batch_size=None, verbose=False):
        ...
        """
        return self._train(
            images, increment=True, group=group, verbose=verbose, batch_size=batch_size
        )

    def _build_shape_model(
        self, shapes, shape_graph, max_shape_components, verbose=False
    ):
    ...
    def _build_deformation_model(self, shapes, deformation_graph, verbose=False):
        return GMRFModel(
            shapes,
            deformation_graph,
            mode="subtraction",
    ...
    def _build_appearance_model(
        self, images, appearance_graph, n_appearance_components, verbose=False
    ):
        if appearance_graph is not None:
            ...
            raise NotImplementedError(
                "The full appearance model is not " "implemented yet."
            )

    def _increment_appearance_model(
        self, images, appearance_graph, appearance_model, verbose=False
    ):
        if appearance_graph is not None:
    ...
    def random_instance(self, scale_index=-1, as_graph=False):
        ...
        return self.instance(shape_weights, scale_index=scale_index, as_graph=as_graph)

    def view_deformation_model(
        self,
        scale_index=-1,
        n_std=2,
        ...
        for e in range(deformation_graph.n_edges):
            ...
            s1 = -self.deformation_models[scale_index].precision[2 * child, 2 * parent]
            s2 = -self.deformation_models[scale_index].precision[
                2 * child + 1, 2 * parent + 1
            ]
            s3 = -self.deformation_models[scale_index].precision[
                2 * child, 2 * parent + 1
            ]
        ...
        if isinstance(deformation_graph, Tree):
            ...
            renderer = PointDirectedGraph(
                mean_shape, deformation_graph.adjacency_matrix
            ).view(
                figure_id=figure_id,
                new_figure=new_figure,
                image_view=image_view,
...
def _compute_minimum_spanning_tree(shapes, root_vertex=0, prefix="", verbose=False):
    ...
    if verbose:
        print_dynamic(
            "{}Deformation graph - Minimum spanning graph " "computed.\n".format(prefix)
        )
    ...
    for k, s in enumerate(aps.scales):
        ...
        scales_info.append(
            lvl_str_tmplt.format(
                s,
                name_of_callable(aps.holistic_features[k]),
                aps.patch_shape[k],
