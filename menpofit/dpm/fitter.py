...
class DPMFitter(object):
    ...
    @staticmethod
    def fast_fit_from_model(image, model, threshold=-10 ** 100, feature_pyramid=None):
        # fit the DPM model to the given image
        padding = (model.maxsize[0] - 1 - 1, model.maxsize[1] - 1 - 1)
        feats, scales = model.feature_pyramid.extract_pyramid(image, model.interval, padding)
        # feats, scales = hog_feature_pyramid(image, model.interval, 4, padding)  # model.feature_pyramid.extract_pyramid(image, model.interval, padding)
        ...
        for level, feat in feats.iteritems():  # for each level in the feature pyramid
            ...
            for c, component in enumerate(model.components):
                ...
                if xs.shape[0] > 0:
                    xy = DPMFitter._old_backtrack(xs, ys, tree, ix, iy, fsz, scale, padding)
    ...
    def fit_with_bb(image, model, threshold=-10 ** 100, bbox=dict(), overlap=0, id_=0, label=0, qp=None,
        ...
        padding = (model.maxsize[0] - 1 - 1, model.maxsize[1] - 1 - 1)
        feats, scales = feature_pyramid.extract_pyramid(image, model.interval, padding)
        # feats, scales = _feature_pyramid(image, model.interval, model.sbin, padding)
        # feats = feature_pyramid['feats']
        # scales = feature_pyramid['scales']
            ...
            for c, component in enumerate(model.components):
                ...
                if latent:
                    ...
                    for cv in tree.vertices:
                        ...
                        ovmask[cv] = DPMFitter._test_overlap(fsz, feat, scales[level], padding, image.shape,
                                                             bbox['box'][cv, :], overlap)
                ...
                for i, ov in ovmask.iteritems():
                    ...
                    # mask = ov  # 1000**(1-ov)
                    # mask = mask/np.max(mask) # can not be -np.inf because of using distance transform
                ...
                if xs.shape[0] > 0:
                    ...
                    xys, ids, ex_filters, ex_defs = DPMFitter._batch_backtrack(xs, ys, tree, ixs, iys, fsz, scale,
                                                                               padding, level, feat, anchors, label,
                                                                               id_)
    ...
    def fit_with_bb_less_memory(image, model, threshold=-10 ** 100, bbox=dict(), overlap=0, id_=0, label=0, qp=None,
        ...
        for level, feat in feats.iteritems():  # for each level in the pyramid
            ...
            for c, component in enumerate(model.components):
                ...
                for i in range(xs.shape[0]):
                    x, y = xs[i], ys[i]
                    xy = DPMFitter._backtrack(x, y, tree, ixs, iys, fsz, scale, padding, ex, True, level, filter_index,
                                              def_index,
                                              feat, anchors)
                    detection_info = dict()
                    detection_info['level'] = level
    ...
    @staticmethod
    def _batch_backtrack(xs, ys, tree, ixs, iys, fsz, scale, pyra_pad, level=-1, feat=None, anchors=None, label=-1,
                         ex_id=-1):
        r"""
        Backtrack the solution from the root to the children and return the detection coordinates for each part.
...
def hog_feature_pyramid(im, m_inter, sbin, pyra_pad):
